...
class LoadAnnotationsDirect:
    ...
    def __call__(self, results):
        pan_mask = np.array(Image.open(results['ann'])).astype(
        ...
        for instance_id in np.unique(pan_mask):  # 0,1...n object id
            ...
            if instance_id == 0:  # no segmentation area
                ...
                try:
                    ...
                    print(f"Error: instance_id{instance_id} out of {len(objects_info)} in image{results['ann']}")
                    flag = False
                    # category = 'background'
                    # gt_semantic_seg[pan_mask == instance_id] = cates2id(
                    #     category)
        ...
        if flag:
            ...
            if len(
                ...
                if self.is_instance_only:
                    gt_masks.masks = np.delete(gt_masks.masks,
