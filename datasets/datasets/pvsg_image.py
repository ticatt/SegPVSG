            ...
            try:
                if self.test_mode:
                    ...
                    if data is not None:
                        ...
                        print(f"error in idx{idx}, retry sample another img")
                        idx = np.random.randint(len(self))
            
            except Exception as e:
                # 捕获其他异常（如索引越界、数据损坏）
                print(f"Error processing index {original_idx} (retry idx={idx}): {str(e)}")
                idx = np.random.randint(len(self))
        raise RuntimeError(
            f"Failed to fetch valid data after {max_retries} retries. "
            f"Original index: {original_idx}, Last tried index: {idx}"
        )
