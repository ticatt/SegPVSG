...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if gauss_weight is not None:
            gauss_weight = gauss_weight.unsqueeze(1).unsqueeze(1).expand(
                -1,
                self.num_heads,
                tgt_len,
                -1,
            ).reshape(*attn_weights.shape)
    ...
    def _in_proj(self, input, start=0, end=None):
        ...
        bias = None if self.in_proj_bias is None else self.in_proj_bias[start:end]
        return F.linear(input, weight, bias)

    def reorder_incremental_state(self, incremental_state, new_order):
        input_buffer = self._get_input_buffer(incremental_state)
        if input_buffer is not None:
            for key in input_buffer.keys():
                input_buffer[key] = input_buffer[key].index_select(0, new_order)
            self._set_input_buffer(incremental_state, input_buffer)

    def _get_input_buffer(self, incremental_state):
        return incremental_state.setdefault(self, {})

    def _set_input_buffer(self, incremental_state, buffer):
        incremental_state[self] = buffer
