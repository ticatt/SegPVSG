...
class Mask2FormerVideoCustom(SingleStageDetector):
    ...
    def simple_test(self, img, img_metas, ref_img, ref_img_metas, **kwargs):
        ...
        # mask_cls_results, mask_pred_results, query_feats = self.panoptic_head.simple_test_with_query(video_x, ref_img_metas, **kwargs)
        pred_logits = []
        mask_pred_list = []
        query_pred_list = []
        mask_feature_list = []
        for i in range(video_x[0].size(0)):
            cur_frame_feas = [video_x[0][i].unsqueeze(0), video_x[1][i].unsqueeze(0),
                              video_x[2][i].unsqueeze(0), video_x[3][i].unsqueeze(0)]
            mask_cls_results, mask_pred_results, query_fea, mask_features = self.panoptic_head.simple_test_with_query(
                cur_frame_feas, ref_img_metas, **kwargs)
            pred_logits.append(mask_cls_results.squeeze())
            mask_pred_list.append(mask_pred_results.squeeze())
            query_pred_list.append(query_fea.permute(0, 2, 1).squeeze())
            mask_feature_list.append(mask_features.squeeze(0).squeeze(0))
        ...
        out_masks.append(mask_pred_list[0])
        out_embds.append(query_pred_list[0])
        out_mask_features.append(mask_feature_list[0])

        for i in range(1, len(pred_logits)):
            indices = self.match_from_embds(out_embds[-1], query_pred_list[i])

            out_logits.append(pred_logits[i][indices, :])
            out_masks.append(mask_pred_list[i][indices, :, :])
            out_embds.append(query_pred_list[i][indices, :])
            out_mask_features.append(mask_feature_list[i])

        out_logits = sum(out_logits) / len(out_logits)
        out_logits = out_logits.unsqueeze(0)  # -> 1, n, c
        out_masks = torch.stack(out_masks, dim=0).unsqueeze(0)  # n h w -> 1, t ,n, h w
        out_embds = sum(out_embds) / len(out_embds)
        out_embds = out_embds.unsqueeze(0) # -> 1, n, c 
        ...
        for frame_id in range(num_frame):
            ...
            result = self.panoptic_fusion_head.simple_test_with_query(
                out_logits,
                out_masks[:, frame_id],
                out_embds,
            ...
            for i in range(len(result)):
                ...
                result[i]["mask_features"] = out_mask_features[frame_id].detach().cpu().numpy()

                if "query_feats" in result[i]:
                    for idx, query_feat_list in result[i]['query_feats'].items():
                        result[i]['query_feats'][idx] = [x.detach().cpu().numpy() for x in query_feat_list]
