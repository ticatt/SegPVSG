...
from .maskformer_video_head import MaskFormerVideoHead


@HEADS.register_module()
class Mask2FormerVideoHead(MaskFormerVideoHead):
    ...
    def __init__(self,
        ...
        self.loss_split_th_st = loss_split_thing_stuff
        self.class_weight = loss_cls.class_weight
        # one cls loss, three seg losses
        self.loss_cls = build_loss(loss_cls)
        self.loss_mask = build_loss(loss_mask)
        self.loss_dice = build_loss(loss_dice)
        if loss_sem_seg is not None:
            self.loss_sem_seg = build_loss(loss_sem_seg)
            self.sem_seg_head = nn.Conv2d(feat_channels, num_stuff_classes + num_things_classes, 1)
        else:
            self.loss_sem_seg = None
    ...
    def loss_single(self, cls_scores, mask_preds, gt_labels_list, gt_masks_list, img_metas):
        ...
        mask_targets = torch.cat(mask_targets_list, dim=0)  # bug -- size do not match ?
        # shape (batch_size, num_queries)
    ...
    def forward_head_video(self, decoder_out, mask_feature, attn_mask_target_size):
        ...
        decoder_out = self.transformer_decoder.post_norm(decoder_out)  # DetrTransformerDecoder
        decoder_out = decoder_out.transpose(0, 1)
        cls_pred = self.cls_embed(decoder_out)  # ([2, 100, 256]) ->  ([2, 100, 62])
        mask_embed = self.mask_embed(decoder_out)  # torch.Size([2, 100, 256]) （bs, 100, c)
        mask_pred = torch.einsum('bqc,btchw->btqhw', mask_embed, mask_feature)  
        # mask_feature torch.Size([2, 2, 256, 96, 120]) (bs,nf,c,h,w) -- detr_decoder (mask embed) + pixel decoder (mask features) -- result merge
    ...
    def forward(self, feats, img_metas, return_query=False):
        """Forward function.
        ...
        mask_features, multi_scale_memorys = self.pixel_decoder(feats) # feats: list of multi-scale feature: each shape is (b*T, c, h, w)
        bs_nf, _c, _h, _w = mask_features.size()
        assert batch_size * num_frame == bs_nf
        mask_features = mask_features.reshape((batch_size, num_frame, _c, _h, _w))
        for idx in range(len(multi_scale_memorys)):
            bs_nf, _c, _h, _w = multi_scale_memorys[idx].size()
            assert batch_size * num_frame == bs_nf  ## assert bs == b * T or (b * nf)
            multi_scale_memorys[idx] = multi_scale_memorys[idx].reshape((batch_size, num_frame, _c, _h, _w))
        ...
        for i in range(self.num_transformer_feat_level):
            ...
            mask = decoder_input.new_zeros((batch_size, num_frame)+multi_scale_memorys[i].shape[-2:], dtype=torch.bool)
        ...
        query_feat = self.query_feat.weight.unsqueeze(1).repeat((1, batch_size, 1))  # [100, 2, 256]
        ...
        cls_pred, mask_pred, attn_mask = self.forward_head_video( 
            query_feat, mask_features, multi_scale_memorys[0].shape[-2:])   # mask_features [2, 2, 256, 96, 120]
        cls_pred_list.append(cls_pred)
        mask_pred_list.append(mask_pred)

        # add semantic seg loss
        if self.training and self.loss_sem_seg is not None:
            batch_size, num_frame, c, h, w = mask_features.size()
            sem_seg_pred = self.sem_seg_head(mask_features.reshape((batch_size*num_frame, c, h, w)))
        ...
        if self.training and self.loss_sem_seg is not None:
            return cls_pred_list, mask_pred_list, sem_seg_pred
        elif self.training and self.loss_sem_seg is None:
    ...
    def forward_train(self,
        ...
        # forward
        all_cls_scores, all_mask_preds, sem_seg_pred = self.forward(feats, img_metas)
        ...
        if self.loss_sem_seg is not None:
            # dense semantic loss as aux loss.
            gt_semantic_seg = gt_semantic_seg.flatten(0, 2).long()
            sem_seg_pred = F.upsample(sem_seg_pred, size=gt_semantic_seg.size()[1:], mode='bilinear')
            b_t, c, h, w = sem_seg_pred.size()
            sem_seg_pred = sem_seg_pred.view(-1, c, h*w).permute(
                    0, 2, 1).reshape(-1, c)
            gt_semantic_seg = gt_semantic_seg.view(-1)

            sem_seg_loss = self.loss_sem_seg(sem_seg_pred, gt_semantic_seg, ignore_index=255)
            losses['sem_seg_loss'] = sem_seg_loss
    ...
    def loss(self, all_cls_scores, all_mask_preds, gt_labels_list,
        ...
        if self.loss_split_th_st:
            ...
            stuff_scores = [all_cls_score[:, -self.num_stuff_classes:, :] for all_cls_score in all_cls_scores]
            stuff_mask_preds = [all_mask_pred[:, :,-self.num_stuff_classes:, :, :] for all_mask_pred in all_mask_preds]

            thing_scores = [all_cls_score[:, :-self.num_stuff_classes, :] for all_cls_score in all_cls_scores]
            ...
            losses_cls_stuff, losses_mask_stuff, losses_dice_stuff = multi_apply(
                ...
                stuff_gt_labels_list, stuff_gt_masks_list, img_metas_list)

            losses_cls_thing, losses_mask_thing, losses_dice_thing = multi_apply(
                self.loss_single, thing_scores, thing_mask_preds,
                thing_gt_labels_list, thing_gt_masks_list, img_metas_list)
    ...
    def simple_test_with_query(self, feats, img_metas, **kwargs):
        """Test with returned query
        Args:
        ...
        Returns:
            ...
            - mask_pred_results (Tensor): Mask logits, shape \
                (batch_size, num_queries, h, w).
        """
        all_cls_scores, all_mask_preds, query_fea, mask_features = self.forward(feats, img_metas, return_query=True)
        ...
        return mask_cls_results, mask_pred_results, query_fea, mask_features