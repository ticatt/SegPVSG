...
class Mask2FormerHeadCustom(MaskFormerHead):
    ...
    def simple_test_with_query(self, feats, img_metas, **kwargs):
        ...
        Returns:
            ...
            - mask_pred_results (Tensor): Mask logits, shape \
                (batch_size, num_queries, h, w).
        """
        all_cls_scores, all_mask_preds, query_feats, mask_features = self.forward(feats, img_metas,return_query=True)
        mask_cls_results = all_cls_scores[-1]
        mask_pred_results = all_mask_preds[-1]
        
        query_feats = torch.unsqueeze(query_feats, dim=0) # expand dim 
        ...
        return mask_cls_results, mask_pred_results, query_feats, mask_features