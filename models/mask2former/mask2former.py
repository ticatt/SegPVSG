...
class Mask2FormerCustom(SingleStageDetector):
    ...
    def simple_test(self, imgs, img_metas, **kwargs):
        ...
        mask_cls_results, mask_pred_results, query_feats, mask_features = self.panoptic_head.simple_test_with_query(
            feats, img_metas, **kwargs) # modify -- use simple_test_with_query to return que_fea with mask!
        ...
        feature_type = getattr(self, 'feature_type', 'query')
        if feature_type == 'mask':
            results[0]['mask_features'] = mask_features.detach().cpu().numpy()
