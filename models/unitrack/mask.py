...
class MaskAssociationTracker(AssociationTracker):
    ...
    def extract_emb(self, img, obs):
        ...
        embs = []
        scales = obs.flatten(1).sum(dim=1).detach().cpu().numpy()
        
        for ob, scale_value in zip(obs, scales):
            obfeat = ob*feat
            if scale_value > 0:
                if scale_value > self.tracker_cfg.mots.max_mask_area:
                    scale_factor = np.sqrt(self.tracker_cfg.mots.max_mask_area/scale_value)
                else:
                    scale_factor = 1
                norm_obfeat = F.interpolate(
                    obfeat,
                ...
                embs.append(emb.contiguous())
            else: 
                embs.append(torch.randn(d, template_scale, device=feat.device))
        return obs, embs
