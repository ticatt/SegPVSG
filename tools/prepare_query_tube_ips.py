...
from models.unitrack.test_mots_from_mask2former import IPSSequenceWriter, eval_seq
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='start offset for sharded video processing')
    parser.add_argument(
        '--video-step',
        type=int,
        default=1,
        help='step size for sharded video processing')
    parser.add_argument(
        '--skip-existing',
        action='store_true',
        help='skip video if target feature file already exists')
...
def stream_ips_sequence(model, data_loader, data_cfg, tracker_cfg, classes,
                        save_root, feature_type):
    model.eval()
    writer = IPSSequenceWriter(
        data_cfg=data_cfg,
    ...
    for data in data_loader:
        with torch.no_grad():
            frame_outputs = model(return_loss=False, rescale=True, **data)
        for frame_output in frame_outputs:
            writer.process(frame_output)
            progress_bar.update()
    writer.finalize()
def main():
    args = parse_args()
    ...
    for i, video_name in enumerate(video_names):
        ...
        if args.skip_existing and osp.exists(target_file):
            print(f'Skip video {video_name} {i}/{len(video_names)}', flush=True)
            continue

        if not osp.exists(save_root):
            os.makedirs(save_root)
        
        print(f'Inference for video {video_name} {i}/{len(video_names)}', flush=True)
