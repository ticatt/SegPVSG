#!/usr/bin/env bash
# Extract VPS tube features and forward remaining arguments to the Python entrypoint.


usage() {
  cat <<'EOF_HELP'
Usage: bash scripts/utils/prepare_feats_vps.sh [SCRIPT OPTIONS] [FEATURE OPTIONS]
...
Common feature options:
  ...
  --work-dir DIR             Default: outputs/features/vps_train_vf
...
unchanged to tools/prepare_query_tube_vps.py.
EOF_HELP
}
GPU=0
CPU_THREADS=16
...
python tools/prepare_query_tube_vps.py \
  configs/mask2former_vps/mask2former_video_r50_single_video_test.py \
  checkpoints/mask2former_r50_vps/epoch_8.pth \
  --work-dir outputs/features/vps_train_vf \
  --split train \
  --feature-type mask \
  --launcher none \
  --skip-existing \
